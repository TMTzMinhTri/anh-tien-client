...
import { Spinner, Container, Col, Row, Table, Button, Modal, ModalHeader, ModalBody, ModalFooter, Form, FormGroup, Label, Input } from 'reactstrap';
import './index.css'
...
export const DetailsUserScreen: React.FC<{}> = () => {
    ...
    const [userInput, setUserInput] = React.useState<{ total: number, note: string }>({ total: 0, note: '' })
    ...
    return <Container>
        ...
        <Modal isOpen={openPopup} toggle={() => setOpenPopup(false)} fade>
            <ModalHeader toggle={() => setOpenPopup(false)}>Modal title</ModalHeader>
            <ModalBody>
                <Form>
                    <FormGroup>
                    ...
                    <FormGroup>
                        ...
                        <Input type="textarea" name="note" id="note" value={userInput.note} onChange={handleChange} />
                    </FormGroup>
                </Form>
            </ModalBody>
            <ModalFooter>
                <Button color="primary" onClick={() => handlePayTheMoney()} > Xạc nhận</Button>{' '}
                <Button color="secondary" onClick={() => setOpenPopup(false)}> Huỷ</Button>
            </ModalFooter>
        </Modal>
    </Container>
}
    ...
    return <Col md={3}>
        {userInfo !== null
            ? <div className="d-flex flex-column">
                ...
                <Button color="danger" onClick={() => setOpenPopup(true)}> Đóng tiền</Button>
            </div>
                        ...
                        <Table hover size="sm">
                            ...
                            <tbody>
                                {userInfo.history.map((item, index) => {
                                    return <tr key={`history_pay_${index}`}>
